...
export default function Header(props){
  ...
  const tabs = (
    ...
    {routes.map((route,index)=>(
      <Tab key={`${route}${index}`} className={classes.tab} component={Link} to={route.link} label={route.name} aria-owns={route.ariaOwns} aria-haspopup={route.ariaPopup} onMouseOver={route.mouseOver}></Tab>
                ...
                {menuOptions.map((option,i)=>{return <MenuItem key={`${option}${i}`} component={Link} to={option.link} classes={{root:classes.menuItem}} onClick={(event)=>{handleMenuItemClick(event,i);props.setValue(1);handleClose()}} selected={i===props.selectedIndex && props.value===1}>{option.name}</MenuItem>})}
                </Menu>
    </React.Fragment>
  );
  ...
  const drawer = (
    ...
    {routes.map(route => (
      ...
      <ListItem className={{root:classes.drawerItemEstimate,selected:classes.drawerItemSelected}} divider button onClick={()=>{setOpenDrawer(false);props.setValue(5)}} component={Link} to="/estimate" selected={props.value===5}>
    ...
    return (
        ...
        <ElevationScroll>
            <AppBar position="fixed" className={classes.appBar}>
                ...
                <Button component = {Link} disableRipple to ="/" className={classes.logoContainer} onClick={()=>{props.setValue(0)}}><img src={logo} clasName={classes.logo} alt="Company logo"></img></Button>
                {matches ? drawer : tabs }
                </Toolbar>
            </AppBar>
            </ElevationScroll>
